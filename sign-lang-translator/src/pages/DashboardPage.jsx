/**
 * pages/DashboardPage.jsx — Enhanced with live metrics, AI pipeline, and session tracking.
 * All existing WebSocket/camera/speech functionality preserved intact.
 */
...
export default function DashboardPage() {
  ...
  const { fps, sessionTimeFormatted, predictionCount, recordPrediction } = useSessionMetrics({
    enabled:    isStreaming,
    confidence,
  });

  // Backend sends ASL signs — treat each as a token
  const handleNewLetter = useCallback((word, conf = 0.85) => {
    setLetters((prev) => {
      const spacer = prev.length > 0
        ? [{ id: Date.now() + Math.random() - 1, letter: ' ' }]
        : [];
      return [...prev, ...spacer, { id: Date.now() + Math.random(), letter: word }];
    });
    setConfidence(conf);
    setCurrentSign(word);
    recordPrediction();
  ...
  // Let TranslatorView bubble up its streaming state
  ...
  return (
    ...
    >
      <div className="page-wrapper">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
              <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--neon-cyan)', letterSpacing: '0.12em' }}>// MODULE: 01</span>
            </div>
            <h1 className="text-display-sm" style={{ color: 'var(--on-surface)', marginBottom: 2 }}>
              Live <span className="text-gradient-cyan">Translation</span>
            </h1>
            <p className="font-mono-label" style={{ color: 'var(--on-surface-variant)' }}>
              ASL → Text · Real-time PyTorch inference
            </p>
          </motion.div>
          ...
          >
            ...
            {NAV_ACTIONS.map(({ icon: Icon, label, id }) => (
              <GlowButton key={id} id={id} variant="ghost" size="sm" aria-label={label} className="!px-3 !py-2">
        ...
        <hr className="laser-divider mb-6" aria-hidden="true" />

        {/* ── Live Metrics Panel ── */}
        <LiveMetricsPanel
          modelActive={isStreaming}
          fps={fps}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-5 items-start">
          ...
          >
            <GlassCard animate={false} glow="cyan" className="p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono-label" style={{ color: 'var(--on-surface-variant)', textTransform: 'uppercase' }}>
                    Camera Feed
                  </span>
                </div>
                <span className="font-mono-label text-xs" style={{ color: 'rgba(0,229,255,0.7)' }}>
                  TRACKING ACTIVE
                </span>
              </div>
            ...
            {/* AI Pipeline below camera */}
            <AIPipeline active={isStreaming} />
          </motion.div>
